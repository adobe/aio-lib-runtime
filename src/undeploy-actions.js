...
async function undeployActions(config, eventEmitter) {
    if (!config.app.hasBackend) throw new Error('cannot undeploy actions, app has no backend')
    ...
    await undeployWsk(packageName, manifest, owOptions, eventEmitter.emit.bind(eventEmitter, 'progress'))

    eventEmitter.emit('end', taskName)
